        ...
        <main className="px-4 sm:px-6 lg:px-8 py-8 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
            <div className="bg-white p-1 rounded-xl shadow-sm border border-gray-200 inline-flex self-start md:self-auto">
                <button onClick={() => setLeaderboardType('receivers')} className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold transition-all ${leaderboardType === 'receivers' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-50'}`}><Medal className="w-4 h-4" />Top Receivers</button>
                <button onClick={() => setLeaderboardType('givers')} className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold transition-all ${leaderboardType === 'givers' ? 'bg-purple-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-50'}`}><Gift className="w-4 h-4" />Top Givers</button>
            </div>
            <div className="flex items-center gap-1 bg-gray-200/50 p-1 rounded-lg self-start md:self-auto">
                {['week', 'month', 'all'].map((t) => (
                    <button key={t} onClick={() => setTimeFilter(t)} className={`px-3 py-1.5 rounded-md text-xs font-bold capitalize transition-all ${timeFilter === t ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>{t === 'all' ? 'All Time' : `This ${t}`}</button>
                ))}
            </div>
          </div>
          ...
          {isLoading ? (
            <div className="h-64 flex flex-col items-center justify-center text-gray-400">
                <Loader2 className="w-8 h-8 animate-spin mb-2 text-blue-500" /><p>Calculating scores...</p>
            </div>
          ) : (
            <>
                {/* --- CHỈ HIỆN PODIUM KHI KHÔNG TÌM KIẾM --- */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-8 mb-12 items-end animate-in fade-in slide-in-from-bottom-4 duration-700">
                    ...
                    <div key={person.id} className={`relative group ${person.rank === 1 ? 'md:order-2 md:-mt-8' : person.rank === 2 ? 'md:order-1' : 'md:order-3'}`}>
                        <div className={`bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300 cursor-pointer text-center relative overflow-hidden ${person.isMe ? 'ring-2 ring-offset-2 ring-blue-500' : ''}`}>
                            ...
                            <div className="relative inline-block mb-4 mt-2">
                                <Avatar className={`w-20 h-20 lg:w-24 lg:h-24 border-4 ${person.rank === 1 ? 'border-yellow-100' : person.rank === 2 ? 'border-gray-100' : 'border-orange-100'}`}><AvatarImage src={person.avatar} /><AvatarFallback>{person.name.charAt(0)}</AvatarFallback></Avatar>
                                <div className={`absolute -bottom-3 left-1/2 transform -translate-x-1/2 w-8 h-8 rounded-full flex items-center justify-center font-bold text-white text-sm border-4 border-white shadow-md ${person.rank === 1 ? 'bg-yellow-500' : person.rank === 2 ? 'bg-gray-500' : 'bg-orange-500'}`}>{person.rank}</div>
                            </div>
                            ...
                            <div className={`inline-flex flex-col items-center justify-center px-6 py-2 rounded-xl bg-gray-50 group-hover:bg-${primaryColor}-50 transition-colors`}>
                                <span className={`text-2xl font-bold text-${primaryColor}-600`}>{person.points}</span>
                                <span className="text-xs text-gray-500 font-medium">{leaderboardType === 'receivers' ? 'Received' : 'Given'}</span>
                            </div>
                        </div>
                    </div>
                    ))}
                    ...
                    <div className="divide-y divide-gray-100">
                        {displayOthers.length === 0 ? (
                            <div className="p-8 text-center text-gray-400">
                                {searchQuery ? `No colleagues found matching "${searchQuery}"` : "No data yet."}
                            </div>
                        ) : (
                            displayOthers.map((person) => (
                            <div key={person.id} className={`grid grid-cols-12 gap-4 px-6 py-4 hover:bg-gray-50 transition-colors items-center ${person.isMe ? 'bg-blue-50/60' : ''}`}>
                                <div className="col-span-2 sm:col-span-1"><span className={`text-sm font-bold w-6 h-6 flex items-center justify-center ${person.rank <= 3 ? 'text-yellow-600 bg-yellow-50 rounded-full' : 'text-gray-500'}`}>#{person.rank}</span></div>
                                <div className="col-span-6 sm:col-span-5 flex items-center gap-3">
                                    <Avatar className="w-9 h-9 border border-gray-100"><AvatarImage src={person.avatar} /><AvatarFallback>{person.name.charAt(0)}</AvatarFallback></Avatar>
                                    <div className="min-w-0">
                                        <div className="font-semibold text-gray-900 flex items-center gap-2 text-sm">{person.name} {person.isMe && <span className="bg-blue-100 text-blue-700 text-[10px] px-1.5 py-0.5 rounded border border-blue-200 font-bold">YOU</span>}</div>
                                        <div className="sm:hidden text-xs text-gray-500 mt-0.5">{person.department}</div>
                                    </div>
                                </div>
                                <div className="hidden sm:block sm:col-span-3"><span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">{person.department}</span></div>
                                <div className="col-span-3 sm:col-span-2 text-right"><span className={`text-sm font-bold ${leaderboardType === 'receivers' ? 'text-blue-600' : 'text-purple-600'}`}>{person.points}</span></div>
