...
const UserProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const calculateStats = async () => {
        const { count: receivedCount } = await supabase.from('kudos_receivers').select('*', { count: 'exact', head: true }).eq('user_id', id);
        const { count: givenCount } = await supabase.from('kudos').select('*', { count: 'exact', head: true }).eq('sender_id', id);
    ...
    const fetchPosts = async () => {
      ...
      try {
        ...
        if (activeTab === 'received') {
            ...
            rawData = Array.from(uniqueMap.values()).sort((a, b) => 
                        ...
                        {loadingPosts ? <div className="text-center py-10"><Loader2 className="animate-spin w-6 h-6 mx-auto"/></div> : 
                         posts.length > 0 ? (
                            <div className="space-y-6">
                                {posts.map(post => (
                                    <PostItem key={post.id} post={post} onUpdate={() => {}} onDelete={() => {}} />
                                ))}
                            </div>
                         ) : (
