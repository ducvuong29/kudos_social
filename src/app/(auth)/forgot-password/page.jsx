...
import { Mail, Eye, EyeOff, ArrowLeft, RotateCcw, CheckCircle, Loader2 } from 'lucide-react';
import { createClient } from '@/lib/supabase/client';
import { useRouter, useSearchParams } from 'next/navigation';
import Link from 'next/link';

// --- 1. VIEW GỬI LINK (Tách ra ngoài) ---
const ForgotPasswordView = ({ email, setEmail, loading, message, handleSendResetLink }) => (
  <div className="w-full max-w-md">
    <div className="bg-white rounded-3xl shadow-2xl p-8">
      ...
      {message.content && (
        <div className={`mb-6 p-4 rounded-xl text-sm font-medium text-center ${message.type === 'success' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
          {message.content}
        </div>
      )}
      ...
      <div className="space-y-6">
        ...
        <Link href="/login" className="w-full flex items-center justify-center gap-2 text-blue-600 font-semibold hover:text-blue-700 transition-all cursor-pointer py-2">
...
const SetNewPasswordView = ({ 
  ...
  loading, message, handleUpdatePassword 
}) => {
  ...
  return (
    <div className="w-full max-w-md">
      <div className="bg-white rounded-3xl shadow-2xl p-8">
        ...
        <p className="text-gray-600 mb-8">Almost there! Choose a strong password to secure your account.</p>
                ...
                <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    className={`h-full transition-all duration-300 ${
                      passwordStrength < 40 ? 'bg-red-500' : passwordStrength < 70 ? 'bg-yellow-500' : 'bg-blue-600'
                    }`}
...
const ForgotPasswordContent = () => {
  ...
  const handleSendResetLink = async () => {
    if (!email) return setMessage({ type: 'error', content: 'Vui lòng nhập email.' });
    setLoading(true);
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: redirectUrl });
      ...
      setMessage({ type: 'success', content: 'Link khôi phục đã được gửi! Vui lòng kiểm tra email của bạn.' });
    } catch (error) {
  ...
  const handleUpdatePassword = async () => {
    if (!newPassword || !confirmPassword) return setMessage({ type: 'error', content: 'Vui lòng nhập đầy đủ thông tin.' });
    if (newPassword !== confirmPassword) return setMessage({ type: 'error', content: 'Mật khẩu xác nhận không khớp.' });
    ...
    try {
      ...
      setMessage({ type: 'error', content: 'Lỗi đổi mật khẩu: ' + error.message });
    } finally {
      setLoading(false);
    }
  };
      ...
      <header className="bg-white border-b border-gray-200 py-4 px-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <Link href="/" className="text-xl font-bold text-gray-900">Kudos Social</Link>
          </div>
          <Link href="/login" className="bg-blue-600 text-white px-6 py-2 rounded-lg font-semibold hover:bg-blue-700 transition-all cursor-pointer">
            Sign In
          </Link>
        </div>
      </header>
