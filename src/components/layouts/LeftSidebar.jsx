...
import { Home, Trophy, LogOut, User, Users, Settings, Moon, Sun, Globe, ChevronDown, ChevronUp } from 'lucide-react';
...
const LeftSidebar = () => { 
  ...
  const NavItem = ({ href, icon: Icon, label }) => (
    <Link href={href}>
      <Button variant="ghost" className={`w-full justify-start gap-3 font-semibold cursor-pointer transition-all duration-200
        ${pathname === href 
      ...
      <aside className="w-64 bg-white dark:bg-slate-900 border-r border-gray-200 dark:border-gray-800 flex flex-col hidden md:flex shadow-sm z-10 sticky top-0 h-screen transition-colors duration-300">
        ...
        <div className="p-6 border-b border-gray-100 dark:border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-bold text-xl text-gray-900 dark:text-white transition-colors">
                Kudos<span className="text-blue-600 dark:text-blue-400">Social</span>
            </h1>
          </div>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto scrollbar-hide">
          ...
          <div className="pt-4 mt-2 border-t border-gray-50 dark:border-gray-800">
             ...
             {/* Setting Controls */}
             <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isSettingsOpen ? 'max-h-60 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex bg-white dark:bg-slate-900 rounded-lg p-1 border border-gray-200 dark:border-gray-700">
                            <button 
                                ...
                                className={`flex-1 flex items-center justify-center gap-1 py-1.5 rounded-md text-xs font-bold transition-all ${theme === 'light' ? 'bg-blue-100 text-blue-700 dark:bg-blue-600 dark:text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                            >
                                <Sun size={14}/> {t.light}
                            </button>
                            <button 
                                onClick={() => setTheme('dark')}
                                className={`flex-1 flex items-center justify-center gap-1 py-1.5 rounded-md text-xs font-bold transition-all ${theme === 'dark' ? 'bg-slate-700 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                            >
                                <Moon size={14}/> {t.dark}
                            </button>
                        </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex bg-white dark:bg-slate-900 rounded-lg p-1 border border-gray-200 dark:border-gray-700">
                            <button 
                                ...
                                className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-all ${language === 'vi' ? 'bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-300 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                            >
                                Tiếng Việt
                            </button>
                            <button 
                                onClick={() => changeLanguage('en')}
                                className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-all ${language === 'en' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                            >
                                English
                            </button>
                        </div>
                ...
                <div className="p-4 bg-gray-50/80 dark:bg-slate-800/80 rounded-2xl border border-gray-100/80 dark:border-gray-700 hover:border-blue-100 dark:hover:border-blue-900 transition-colors group">
                    <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold truncate text-gray-900 dark:text-white">{user?.full_name || t.loading}</p>
                            <p className="text-xs text-blue-600 dark:text-blue-400 font-medium">{t.online}</p>
                        </div>
                    </div>
                    <Button onClick={handleLogout} variant="outline" size="sm" className="w-full gap-2 text-gray-600 dark:text-gray-300 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 border-gray-200 dark:border-gray-600 cursor-pointer">
