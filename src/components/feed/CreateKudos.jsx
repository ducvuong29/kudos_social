...
const CreateKudos = ({ onSuccess }) => {
  ...
  const uploadImages = async () => {
    ...
    for (const file of selectedFiles) {
      const fileName = `${Date.now()}_${Math.random().toString(36).substring(2)}.${file.name
        .split('.')
        .pop()}`;
          ...
          <div className="flex-1">
              ...
              {selectedReceivers.map(u => (
                ...
                >
                  ...
                  <Avatar className="w-6 h-6">
                    <AvatarImage src={u.avatar_url || 'https://github.com/shadcn.png'} />
                  </Avatar>
                  {u.full_name}
                  <button
                    onClick={() => setSelectedReceivers(prev => prev.filter(r => r.id !== u.id))}
            ...
            <div className="mb-3 relative group" ref={searchContainerRef}>
              <Input
                ...
                className={`h-14 pl-5 border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-slate-900/50 text-lg rounded-2xl transition-all font-medium dark:text-white dark:placeholder:text-gray-500 ${
                  isSearchActive
            ...
            <Textarea
              ...
              className="mb-4 border-none p-0 text-xl min-h-[80px] resize-none focus-visible:ring-0 placeholder:text-gray-300 dark:placeholder:text-gray-600 font-light leading-relaxed bg-transparent dark:text-white"
            />
            ...
            {showTagSuggestions && (
              <div className="mb-6 p-5 bg-white dark:bg-slate-900 rounded-2xl border-2 border-dashed border-gray-100 dark:border-gray-700 animate-in zoom-in-95">
                <div className="flex items-center gap-2 mb-4 text-xs font-bold text-gray-400 uppercase tracking-wider">
                  <Sparkles size={14} className="text-yellow-500" /> {t.suggestedTags}
                </div>
                  ...
                  {predefinedTags.map(tag => {
                    ...
                    return (
                      ...
                      >
                        {tag}
                        {isSelected && (
                          <Check size={14} className="inline ml-1.5 -mt-0.5 stroke-[3]" />
                        )}
                      </button>
                    );
                  })}
            ...
            <div className="flex justify-between items-center pt-4 border-t border-gray-100 dark:border-gray-700">
              <div className="flex gap-2">
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  className="rounded-full cursor-pointer text-gray-400 dark:text-gray-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 hover:text-emerald-600 transition-colors"
              ...
              <Button
                className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl px-8 py-6 font-bold text-base shadow-xl cursor-pointer shadow-indigo-200 dark:shadow-none active:scale-95 transition-all"
