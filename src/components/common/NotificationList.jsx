...
export const highlightPost = (postId) => {
  // Tìm element theo ID (lưu ý ID trong DOM là post-123)
...
const NotificationList = () => {
  ...
  const handleNotificationClick = (notif) => {
    setShowNotifications(false);
    const targetId = `post-${notif.resource_id}`; // ID của thẻ HTML

    if (pathname === "/") {
      // TRƯỜNG HỢP 1: Đang ở trang chủ -> Gọi hàm highlight luôn
      highlightPost(targetId);
    } else {
      // TRƯỜNG HỢP 2: Đang ở trang khác -> Chuyển hướng kèm Hash
      router.push(`/#${targetId}`);
    }
  };
  useEffect(() => {
    // Kiểm tra xem URL có chứa hash không (ví dụ #post-123)
    const hash = window.location.hash;

    if (hash && hash.startsWith("#post-")) {
      const postId = hash.substring(1); // Lấy "post-123" bỏ dấu #

      // Delay nhẹ 1 chút để đảm bảo DOM đã render xong hết
      // (Khắc phục lỗi cuộn lơ lửng do ảnh chưa load kịp)
      setTimeout(() => {
              ...
              notifications.map((notif) => (
                <div
                  ...
                  className={`p-4 flex gap-3 hover:bg-gray-50 dark:hover:bg-slate-700 cursor-pointer border-b border-gray-50 dark:border-slate-700 last:border-0 transition-colors ${
                    !notif.is_read
                      ? "bg-blue-50/60 dark:bg-blue-900/40 border-l-4 border-l-blue-500"
                      : ""
                  }`}
                >
